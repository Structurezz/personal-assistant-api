...
const log = (0, debug_1.default)('twilio-runtime-handler:dev:route');
const RUNNER_PATH = process.env.NODE_ENV === 'test'
    ? (0, path_1.resolve)(__dirname, '../../dist/dev-runtime/internal/functionRunner')
        ...
        Object.defineProperty(context, 'SERVICE_SID', {
            get: () => {
                if (_serviceSid === undefined) {
                    console.warn((0, chalk_1.default) `{bold.yellow WARNING} at ${(0, getCodeLocation_1.getCodeLocation)({
...
function constructContext({ url, env, logger, baseDir }, functionPath) {
    function getTwilioClient(opts) {
        ...
        return (0, requireFromProject_1.requireFromProject)(baseDir, 'twilio', true)(env.ACCOUNT_SID, env.AUTH_TOKEN, {
            lazyLoading: true,
            ...opts,
        });
    }
...
function constructGlobalScope(config) {
    twilio = (0, requireFromProject_1.requireFromProject)(config.baseDir, 'twilio', true);
    const GlobalRuntime = Runtime.create(config);
    global['Twilio'] = { ...twilio, Response: response_1.Response };
...
function functionPathToRoute(functionPath, config) {
    return function twilioFunctionHandler(req, res, next) {
        ...
        forked.on('message', ({ err, reply, debugMessage, debugArgs = [], crossForkLogMessage, }) => {
            if (debugMessage) {

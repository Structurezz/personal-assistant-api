...
const crossForkLogger_1 = require("./crossForkLogger");
...
process.on('message', ({ functionPath, event, config, path }) => {
    try {
        (0, route_cache_1.setRoutes)(config.routes);
        (0, route_1.constructGlobalScope)(config);
        config.logger = new crossForkLogger_1.CrossForkLogger();
        let context = (0, route_1.constructContext)(config, path);
        sendDebugMessage('Context for %s: %p', path, context);
        context = (0, route_1.augmentContextWithOptionals)(config, context);
